...
36. 后台 RBAC MVP 做到管理员、角色、菜单权限、Controller 资源权限和操作日志；暂不做组织架构、数据范围权限、字段级权限和审批流。
...
| 鉴权 | JWT access token + `user_login_session.session_token` 会话令牌，Redis 保存登录态 / 黑名单 |
...
| 管理端 | `arch-manage` | `com.hopeandsparks.manage` | 后台聚合入口、后台账号权限、操作日志、只读统计看板 | `sys_admin`、`sys_role`、`sys_admin_menu`、`sys_admin_resource`、`sys_admin_resource_category`、`sys_admin_role`、`sys_role_admin_menu`、`sys_role_admin_resource`、`sys_operation_log` |
...
1. `sys_admin`：后台管理员账号。
2. `sys_role`：后台角色。
3. `sys_admin_menu`：后台目录树，使用 `parent_id` 和 `leval` 表达层级。
4. `sys_admin_resource`：后台 Controller 资源，`url` 存 Controller 类上 `@RequestMapping` 的真实基础路径，例如 `/api/v1/manage/users`；不逐条存放方法级接口。
5. `sys_admin_resource_category`：后台资源分类。
6. `sys_admin_role`：管理员与角色关系。
7. `sys_role_admin_menu`：角色与目录关系。
8. `sys_role_admin_resource`：角色与后台 Controller 资源关系。
9. `sys_operation_log`：后台关键操作日志。
...
4. Controller 资源权限，按后台页面对应的 Controller 基础路径控制资源访问；关键写操作仍必须记录操作日志。
...
| `sys_admin` | 后台管理员账号 |
| `sys_role` | 后台角色 |
| `sys_admin_menu` | 后台目录树 |
| `sys_admin_resource` | 后台 Controller 资源 |
| `sys_admin_resource_category` | 后台资源分类 |
| `sys_admin_role` | 管理员与角色关系 |
| `sys_role_admin_menu` | 角色与目录关系 |
| `sys_role_admin_resource` | 角色与后台 Controller 资源关系 |
...
4. 后台权限基于 `sys_admin`、`sys_role`、`sys_admin_menu`、`sys_admin_resource`、`sys_admin_resource_category` 做 RBAC。
5. 管理端关键操作写 `sys_operation_log`。

### 8.3 后台 RBAC MVP 边界

后台 RBAC 做到“能登录、能分角色、能控制菜单、能控制 Controller 资源、能追溯关键操作”即可。
...
1. Access Token 短有效期，`user_login_session.session_token` 作为长有效期会话令牌。
...
5. 接口文档、DTO/VO 与 SQL 表结构冲突时，以 `资源/hope_sparks.sql` 为准。
6. 主键 ID 对前端统一以字符串返回。
7. Controller 按 Maven 业务子模块分组，内部调用本模块 Service。
8. DTO / VO 层负责入参与响应字段，不让前端直接依赖数据库实体和数据库枚举。
...
4. 学习计划、学习拓扑、知识点资源网络。
5. 练习提交、评测报告、薄弱点反哺。
6. Manage 管理员登录、菜单权限、Controller 资源权限、操作日志。
