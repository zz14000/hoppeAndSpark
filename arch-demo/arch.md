...
infrastructure 技术适配：MySQL、Redis（含 Stream）、Chroma、MinIO、Coze、LLM、第三方 API
...
4. Infrastructure 负责 MySQL、Redis（含 Stream）、Chroma、MinIO、Coze、LLM 等技术细节。
...
幂等键可复用 `async_generation_task.idempotent_key` 或业务目标 ID；重试次数落在 `async_generation_task.retry_count` 或对应业务状态字段。
...
application/usecase
infrastructure/redis/stream-publisher
...
后续如生成任务压力变大，可将 `worker/*` 与 Redis Stream consumer 拆成独立进程，API 应用不需要改业务接口。
...
2. `V1` 已包含 `sys_agent_config`、Coze 会话映射字段、Redis Stream 任务追踪字段、社区审核状态字段。
...
2. 基础设施：MySQL、Redis、MinIO、Chroma、Coze 客户端适配。
...
7. Redis（含 Stream）、MinIO、Chroma、Coze 的配置属性类。
